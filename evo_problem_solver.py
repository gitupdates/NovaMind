#!/usr/bin/env python3
"""Evolutionary Python Problem Solver.

This tool combines local linters, formatters and optional offline LLMs to
solve, repair and upgrade Python code. It installs required dependencies on
the fly, so it can run in isolated environments.
"""
...
def _install(pkgs: Iterable[str]) -> None:
    ...
    except subprocess.CalledProcessError as err:  # pragma: no cover - runtime
...
def _run_cmd(cmd: Sequence[str], *, inp: str | None = None, timeout: int = TIMEOUT) -> subprocess.CompletedProcess[str]:
    return subprocess.run(cmd, input=inp, text=True, capture_output=True, timeout=timeout, check=False)
...
def _solve_with_local_llm(prompt: str) -> str | None:
    ...
    try:
        ...
        model_path = os.getenv(LLAMA_MODEL_ENV) or os.getenv(GPT4ALL_MODEL_ENV)
        if model_path and Path(model_path).is_file():
...
class PythonProblemSolver:
    ...
    def solve_problem(self, description: str) -> str:
        ...
        if result is None:
            raise RuntimeError(
                "No local LLM available. Install llama-cpp-python or set OFFLINE_LLM_CMD."
            )
...
def _cli() -> None:
    ...
    s0 = sub.add_parser("solve")
    s0.add_argument("desc")
    s1 = sub.add_parser("repair")
    s1.add_argument("file", type=Path)
    s1.add_argument("--inplace", action="store_true")
    s2 = sub.add_parser("upgrade")
    s2.add_argument("file", type=Path)
    s2.add_argument("--inplace", action="store_true")
